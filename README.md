...
3. **Emails the full issue to paid subscribers**, and publishes free social copy — with subscription billing on Stripe.

No editor reviews an issue before it ships. The [live site](https://usagineko7865-debug.github.io/trendpulse/) shows the free web feed; Pro subscribers receive the full issue by email.
...
│   ├── email_service.py  EmailService — real Brevo REST API (HTTPS)
...
Brevo's HTTP API rather than simulating a send. The only intentional mock in
...
| `BREVO_API_KEY` / `BREVO_SENDER_EMAIL` | [app.brevo.com](https://app.brevo.com) — verify a single sender email (no domain needed), then Settings → SMTP & API → API Keys. Free tier: 300 emails/day. |
...
- Creating a Brevo account and verifying a single sender email address
