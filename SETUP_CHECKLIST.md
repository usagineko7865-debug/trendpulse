...
- [ ] Variables tab → paste in all values from `backend/.env.example` (with your real keys from step 1).
- [ ] **Attach a persistent volume before the first real deploy** — Railway service → right-click the service card → Attach volume → mount path `/app/data` → then set `DB_PATH=/app/data/trendpulse.db` in Variables. **Skipping this wipes all subscriber/issue data on every future deploy** (this actually happened once on 2026-09-15 before it was caught and fixed — see `backend/docker-entrypoint.sh` for why a plain volume mount alone still isn't enough, since Railway mounts volumes owned by root while the app runs as a non-root user).
